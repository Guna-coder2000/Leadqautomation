...
test.describe('Enterprise API Negative Scenarios (404 and 500 Errors)', () => {
  ...
  test('TC-API-001 Verify API correctly handles 404 Not Found errors', async ({ request }) => {
    ...
    await test.step('Step 1: Send GET request to a non-existent endpoint', async () => {
      ...
      const response = await request.get('https://jsonplaceholder.typicode.com/invalid-endpoint-that-does-not-exist');
  ...
  test('TC-API-002 Verify API correctly handles 500 Internal Server errors', async ({ request }) => {
    allure.story('Error Handling');
    ...
    await test.step('Step 1: Send request to an endpoint that triggers a server error', async () => {
      ...
      const response = await request.get('https://dummyjson.com/http/500');
